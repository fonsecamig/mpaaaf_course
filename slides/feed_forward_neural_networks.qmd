...
\nabla_{\widehat{\mathbf y}} \mathscr{L} \cdot \sigma'(\mathbf z^{(L + 1)})
$$
...
& = \boldsymbol \delta^{(l+1)} \mathbf W^{(l+1)} \text{diag}\left(\sigma'(\mathbf z^{(l)})\right)
\end{align*}
...
::: {.fragment}
$$
\frac{\partial \mathscr L}{\partial \mathbf b^{(l)}} = \boldsymbol \delta^{(l)}
$$
:::

...
:::{.callout title="Regression"}
$$\mathscr{L}_{MSE} = \frac{1}{n}\sum_i (y_i - \widehat{y}_i)^2$$
:::

:::{.callout title="Classification"}
- Binary
$$\mathscr{L}_{BCE} = -\frac{1}{n}\sum_i y_i \log(\widehat{y}_i) + (1-y_i)\log(1-\widehat{y}_i)$$
- Multiclass
$$\mathscr{L}_{CE} = -\frac{1}{n}\sum_i \sum_k y_{ik} \log(\widehat{y}_{ik})$$
:::
